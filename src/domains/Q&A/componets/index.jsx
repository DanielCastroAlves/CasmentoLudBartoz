...
import { ContainerAcc, AccordionStyled, AccordionSummaryStyled, AccordionDetailsStyled, TypographyStyled, ListItem, Description, SubList, SubListItem, CustomList, EmprAir, Text } from "./style";
            ...
            A: item.A.map((paragraph) => {
                if (typeof paragraph === "string") {
                    ...
                    const formattedParagraph = {
                        ...
                        additionalInfodescriptionData: paragraph.additionalInfodescriptionData || null,
                        additionalInfodescription: paragraph.additionalInfodescription || null,
                    };
        ...
        <ContainerAcc>
            {formattedData.map((item) => (
                <AccordionStyled key={item.id}>
                    <AccordionSummaryStyled id={`panel${item.id}-header`} onClick={() => handleChangeIndex(item.id)} isExpanded={expandedIndex === item.id}>
                        <div className="icon-typography-container">
                            {expandedIndex === item.id ? (
                                ...
                                <KeyboardArrowDown style={{ fill: "var(--green-olive, #52736b)" }} className="icon" />
                            )}
                            <TypographyStyled className={`typography ${expandedIndex === item.id ? "pergunta-aberta" : ""}`} dangerouslySetInnerHTML={{ __html: item.Q }} />
                            {/* <TypographyStyled className={`typography ${expandedIndex === item.id ? "pergunta-aberta" : ""}`}>{item.Q}</TypographyStyled> */}
                        </div>
                            ...
                            <CustomList>
                                {item.A.map((paragraph, i) => (
                                    <ListItem key={i}>
                                        ...
                                        {paragraph.descriptionData && <Text>{paragraph.descriptionData[0]}</Text>}
                                        ...
                                        {paragraph.additionalInfoemprAir && (
                                            <EmprAir>
                                                ...
                                                <SubList>
                                                    {paragraph.additionalInfoemprAir.slice(1).map((info, k) => (
                                                        <SubListItem key={k} dangerouslySetInnerHTML={{ __html: info }} />
                                                    ))}
                                                </SubList>
                                            </EmprAir>
                                        )}
                                        {paragraph.additionalInfodescriptionData && <Text>{paragraph.additionalInfodescriptionData[0]}</Text>}
